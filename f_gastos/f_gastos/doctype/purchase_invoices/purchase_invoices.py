...
class PurchaseInvoice(BuyingController):
	def __init__(self, *args, **kwargs):
		...
		self.status_updater = [
			{
				"source_dt": "Purchase Invoice items",
	...
	def validate(self):
		...
		self.clear_unallocated_advances("Purchase Invoice Advances", "advances")
			...
			self.status_updater.append(
				{
					...
					where name=`tabPurchase Invoice items`.parent and update_stock = 1)""",
				}
			)
				...
				self.status_updater.append(
					{
						...
						"overflow_type": "receipt",
						"extra_cond": """ and exists (select name from `tabPurchase Invoice`
						where name=`tabPurchase Invoice items`.parent and update_stock=1 and is_return=1)""",
					}
				)
					...
					if provisional_accounting_for_non_stock_items:
						if item.purchase_receipt:
							...
							if expense_booked_in_pr:
								# Intentionally passing Purchase Invoice items to handle partial billing
	...
	def make_precision_loss_gl_entry(self, gl_entries):
		round_off_account, round_off_cost_center = get_round_off_account_and_cost_center(
			self.company, "Purchase Invoices", self.name, self.use_company_roundoff_cost_center
		)
	...
	def get_pr_details_billed_amt(self):
		...
		if pr_details:
			doctype = frappe.qb.DocType("Purchase Invoice items")
			query = (
	...
	def set_tax_withholding(self):
		...
		if self.apply_tds and not self.get("tax_withholding_categorys"):
			self.tax_withholding_category = frappe.db.get_value(
				"Supplier", self.supplier, "tax_withholding_categorys"
			)
			...
			if self.docstatus == 2:
				...
				# Check if outstanding amount is 0 due to debit note issued against invoice
					...
					and frappe.db.get_value(
						"Purchase Invoices", {"is_return": 1, "return_against": self.name, "docstatus": 1}
					)
				):
...
def get_purchase_document_details(doc):
	if doc.doctype == "Purchase Invoices":
		...
		parent_doctype = "Purchase Invoices"
...
def make_debit_note(source_name, target_doc=None):
	...
	return make_return_doc("Purchase Invoices", source_name, target_doc)
...
def make_stock_entry(source_name, target_doc=None):
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Purchase Invoices": {"doctype": "Stock Entry", "validation": {"docstatus": ["=", 1]}},
...
def change_release_date(name, release_date=None):
	if frappe.db.exists("Purchase Invoices", name):
		pi = frappe.get_doc("Purchase Invoices", name)
...
def make_inter_company_sales_invoice(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Purchase Invoices", source_name, target_doc)


def on_doctype_update():
	frappe.db.add_index("Purchase Invoices", ["supplier", "is_return", "return_against"])
